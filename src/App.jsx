  ...
  return (
    <>
      ...
      <ReviewModal
        ...
        targetUserName={listing.userName || 'User'}
...
// Enhanced Search Filters with Listing Type
const SearchFilters = ({ onSearch, onFiltersChange, onSortChange }) => {
  const { t, userLocation } = useLanguage();
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <div>
          ...
          <input
            ...
            className="w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 p-2 border text-sm"
          />
        </div>
        ...
        <div>
          ...
          <select
            ...
            className="w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 p-2 border text-sm bg-white"
          >
            {categories.map(cat => (
              <option key={cat} value={cat === t('allCategories') ? '' : cat}>{cat}</option>
            ))}
          </select>
        </div>
        ...
        <div>
          ...
          >
            ...
            <option value="city">{t('withinCity')}</option>
            <option value="state">{t('withinState')}</option>
        ...
        <div>
          ...
          >
            {listingTypes.map(type => (
              <option key={type} value={type === t('allTypes') ? '' : type}>{type}</option>
            ))}
          </select>
        </div>
...
const Navbar = ({ usingDemoMode, activeTab, onTabChange }) => {
  const { user, userProfile, logout } = useContext(FirebaseContext);
  const { t } = useLanguage();
    ...
    <div className="bg-white shadow-md sticky top-0 z-10">
      ...
      <div className="p-4 border-b border-gray-200">
        <div className="flex justify-between items-center max-w-6xl mx-auto">
          <div className="flex items-center space-x-4">
            <h1 className="text-2xl font-bold text-green-700 font-inter">{t('appName')}</h1>
            {usingDemoMode && (
...
// Note: The rest of the components (TradeOfferModal, MessagingSystem, TradeOffers, etc.)
// would be updated similarly to support the new sale functionality and enhanced features.

// For brevity, I've shown the key enhanced components. The complete implementation
// would include all the previous messaging, trade offers, and other systems
// updated to work with both trade and sale listings.
